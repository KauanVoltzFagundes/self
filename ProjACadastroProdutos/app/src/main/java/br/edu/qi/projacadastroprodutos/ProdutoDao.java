...
public class ProdutoDao {
    private SQLiteDatabase bd_loja;
    private  ConexaoBD conexaoBD;

    public ProdutoDao(Context context){
        this.conexaoBD = new ConexaoBD(context);
        this.bd_loja = conexaoBD.getWritableDatabase();
    }
    public void cadastrarProduto(Produto objProduto){
        ...
        this.bd_loja.insert("tb_produto",null,values);

    }
    public List <Produto> consultarProdutos(){
        List<Produto> encontrados = new ArrayList<>();

        Cursor cursor = bd_loja.query("tb_produto",new String[]{"id","nome","categoria","valor"},
